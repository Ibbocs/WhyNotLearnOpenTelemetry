...
public class StockService
{
    ...
    {
        var userId = Activity.Current?.GetBaggageItem("userId"); //otl header'in dasidigimiz datani oxumagimiz
        ...
        if (isSuccess)
            return ResponseDto<StockCheckAndPaymentProcessResponseDto>.Success(HttpStatusCode.OK.GetHashCode(),
                new StockCheckAndPaymentProcessResponseDto { Description = "ödeme süreci tamamlandı." });
